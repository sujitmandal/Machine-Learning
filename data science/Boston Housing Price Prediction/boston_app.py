...
temp_dataset = pd.read_csv('datasets/boston_data.csv')
validation = pd.read_csv('datasets/boston_test_data.csv')
...
#Inspect the Dataset
#sns.pairplot(temp_dataset[['crim', 'zn', 'indus', 'chas', 'nox','rm','age', 'dis', 'rad','tax','ptratio', 'lstat', 'medv']], diag_kind="kde")
#st.pyplot()
...
try:
    if not os.path.exists('saved model/my_model.h5'):
        ...
        #print("Testing set Mean Absolute Error: {:5.2f} ANN".format(mae))
        #print("Testing set Mean Square Error: {:5.2f} ANN".format(mse))
        ...
        plt.plot(plot_data, predictions, 'r--')
        plt.xlabel('[__Actual],  [----Predicted]')
        plt.ylabel('Price')
        plt.grid(True)
        st.pyplot()
        ...
        #Prediction
        predictions = model.predict(validation).flatten()
        st.title('Prediction')
        st.dataframe(predictions)
